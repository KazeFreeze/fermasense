...
def serial_reader_thread():
    ...
    while not shutdown_event.is_set():
        if not SERIAL_PORT:
            ...
            socketio.emit('mcu_log', {'type': 'error', 'message': 'Serial port not configured. Please select one in Configuration.'})
                ...
                if line:
                    ...
                    if parts[0] == "DATA" and len(parts) == 7:
                        try:
                            ...
                            log_to_csv(MAIN_LOG_FILE, payload,
                                       ['server_time_iso', 'mcu_time_s', 'current_temp', 'set_temp_min', 'set_temp_max', 'state', 'mode'])
                    ...
                    # Expected EQUALIZED: EQUALIZED,TargetTempMin,TargetTempMax,Duration_s
                        ...
                        try:
                            eq_payload = {
                                'server_time_iso': timestamp_iso,
                                'target_temp_min': float(parts[1]), # Assuming this is the target it was equalizing to
                            ...
                            eq_chart_event = {
                                'server_time_iso': timestamp_iso,
                                'target_temp': float(parts[1]), # Using min as the representative target
                                'duration_s': float(parts[3])
                            }
                    ...
                    # Expected STATUS: STATUS,mcu_time,currT,setT_min,setT_max,State,Mode,Freq,isEq,eqTimestamp
                        ...
                        log_type = 'info'
                        if parts[0] == "ERROR": log_type = 'error'
                        elif parts[0] == "WARN": log_type = 'warn' # Or map to 'error' or 'info'
                        elif parts[0] == "CMD_RECV": log_type = 'cmd_recv'
...
def on_connect():
    ...
    emit('available_serial_ports', get_available_serial_ports()) # Send current ports on connect
    if SERIAL_PORT:
        emit('serial_port_status', {'status': 'info', 'message': f'Currently using {SERIAL_PORT}', 'port': SERIAL_PORT})
